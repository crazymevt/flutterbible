...
- **Proximity Search (NEAR):** Use `~` followed by a number to find words near each other. For example, `faith ~10 works` finds verses where "faith" and "works" are within 10 words of each other, in any order. You can chain this: `grace ~5 saved ~5 faith`.
...
- `nt: love` - Searches only in the New Testament.
- `John: light` - Searches only in the book of John.

## Search Index
If you ever notice that search results look strange or contain formatting markup, you can rebuild the search index manually from the Settings menu.
