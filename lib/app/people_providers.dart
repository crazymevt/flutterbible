...
final personSearchResultsProvider =
    FutureProvider<List<BiblePerson>>((ref) async {
...
class PersonEvent {
  ...
  final int? chapter;
  final int? verse;
  PersonEvent(this.eventId, this.title, this.startYear,
      {this.bookName, this.chapter, this.verse});
}
...
final personDetailProvider =
  ...
  Future<BiblePerson?> byId(int? id) async => id == null
      ? null
      : await (store.select(store.biblePeople)..where((p) => p.id.equals(id)))
          .getSingleOrNull();
  ...
  final partnerLinks = await (store.select(store.personPartners)
        ..where((r) =>
            r.personId.equals(personId) | r.partnerId.equals(personId)))
      .get();
  final partnerIds = {
    for (final r in partnerLinks) r.personId == personId ? r.partnerId : r.personId,
  }..remove(personId);

  final childRows = await (store.select(store.biblePeople)
        ..where((p) =>
            p.fatherId.equals(personId) | p.motherId.equals(personId)))
      .get();
  ...
  final groupRows = await store.customSelect(
  ...
  final verses = await (store.select(store.personVerses)
...
final peopleForPassageProvider = FutureProvider.family<List<PersonInPassage>,
...
final currentPassagePeopleProvider =
    FutureProvider<List<PersonInPassage>>((ref) async {
  final book = ref.watch(selectedBookNameProvider);
  final chapter = ref.watch(selectedChapterProvider);
  return ref
      .watch(peopleForPassageProvider((book: book, chapter: chapter)).future);
});
